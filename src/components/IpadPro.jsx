...
const SmartKeyBoard = ({selectedVariant,setSelectedVariant,params,selectedProduct}) => {
  ...
  useEffect(() => {
    ipadWifiConnectivity.map(item => {
      if(item.id === 2){
       return item.price = getPrice + 9000
      } else {
  ...
  return (
    <div>
      ...
      {buttonClose === false && <div>
        ...
        <div className='button-wrapper'>
          {selectedProduct && selectedProduct.map(item => {
            ...
            >
            <img src={item.colorTag} alt='item-color'/>
            {item.color}
            </button>
          })}
        </div>
      ...
      {buttonCloseStorage === false && <>
        ...
        {modelButtonActive.includes('11-inch') && ipadStorage.map((item, index) => {
          return <button 
          key={index}
          onClick={()=>{
        ...
        {modelButtonActive.includes('12.9-inch') && ipadStorage12Inch.map((item, index) => {
          return <button 
          key={index}
          onClick={()=>{
            setActiveButtonStorage(item.storage)
      ...
      <div className='pencil-img-wrapper d-flex justify-content-center flex-column'>
        ...
        <div className='d-flex'>
          {smartFolioData.map((itemButton, index) => {
            ...
            className={smartFolioButton === itemButton.color ? 'smart-folio-button-active' : 'smart-folio-button'}>
              <img src={itemButton.colorImg} alt='color'/>
            </button>
          })}
        </div>
        ...
        <div className='mt-4'>
          <img className='pencil-img'src={KeyboardSmartImg} alt='keyboard'/>
